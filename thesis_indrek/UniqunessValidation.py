...
def testSample(dn, tests, hours = 10, disparity=2, 
        mut_pow=1, gen_size = 50, index = 0, times = 20):
    ...
    for i in range(gen_size):
        ...
        dns.append(dn)
    u_schedule = [(1000, 1800), (1000, 7200)]
    ...
    search.saveResults(True, False, "resultDump", index+3*times)
...
def main():
    ...
    if "s" in args:
        ...
        skips = 0
    for index in range(skips, times):
        print (index)
        dn = getRandomDn(dop, round(dop/10))
        testSample(dn, tests, hours=1, gen_size=100, times=times, index=index+startIndex)
