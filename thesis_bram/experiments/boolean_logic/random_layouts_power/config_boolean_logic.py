...
class experiment_config(config_class):
    ...
    def __init__(self):
        ...
        self.P_max = 8*(0.5*np.max(self.controlrange))
    ...
    def FitnessPower(self, V, I, target):
        '''
        Fitness function that combines correlation (PCC) with
        power consumption (P):

        F = (1 - P/P_max)**3*PCC

        Assumes that the output signal is on electrode 2
        '''
        # Get correlation
        pcc = self.FitnessCorr(I[2], target)

        # Get power consumption
        P = np.abs(np.sum(V*I, axis = 0))
        P = np.mean(P)

        return (1 - P/self.P_max)**3*pcc
