...
# Define 8 electrodes
electrodes = np.zeros((3 + len(cf.controls), 4))
electrodes[0] = cf.electrode_positions[cf.P]
electrodes[1] = cf.electrode_positions[cf.Q]
electrodes[2] = cf.electrode_positions[cf.output]
for ii in range(len(cf.controls)):
    electrodes[3 + ii] = cf.electrode_positions[cf.controls[ii]]

static_electrodes = np.zeros((len(cf.static_controls), 4))
for ii in range(len(cf.static_controls)):
    static_electrodes[ii] = cf.electrode_positions[cf.static_controls[ii]]
...
for i in range(cf.generations):
    ...
    for j in range(cf.genomes):
        ...
        for k in range(len(cf.controls)):
            kmc.electrodes[3 + k, 3] = cf.controlrange[0]*(1 - genePool.pool[j, k]) \
                                       + genePool.pool[j, k]*cf.controlrange[1]
        for k in range(len(cf.static_controls)):
            kmc.static_electrodes[k, 3] = cf.controlrange[0]*(1 - genePool.pool[j, k + len(cf.controls)]) \
        ...
        for k in range(4):
            kmc.electrodes[0, 3] = P[k]*(cf.inputrange[0]*(1 - genePool.pool[j, -1]) + genePool.pool[j, -1]*cf.inputrange[1])
            kmc.electrodes[1, 3] = Q[k]*(cf.inputrange[0]*(1 - genePool.pool[j, -1]) + genePool.pool[j, -1]*cf.inputrange[1])
            kmc.update_V()
            kmc.simulate_discrete(prehops = prehops)
            for l in range(cf.avg):
                kmc.simulate_discrete(hops = hops)
                output[k*cf.avg + l] = kmc.current[2]
